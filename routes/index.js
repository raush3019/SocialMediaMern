...
router.post("/forgetPassword", function (req, res, next) {
  ...
  userDetail
    ...
    .then((userFound) => {
      if (userFound === null) {
        return res.send(
          "<h1 style='color: red; position: absolute;left: 50%;transform: translateX(-50%); margin-top: 5vmax; text-align: center;'>User Not Found <br> <a style='color:black;' href='/forgetPass'>Go Back</a></h1>"
        );
      }
      // ***********************************
      ...
      var transporter = nodemailer.createTransport({
        service: "gmail",
        auth: {
          user: "raushankumar3019@gmail.com",
          pass: "elaqukecrbexjvfv",
        },
      });

      var mailOptions = {
        from: "raushankumar3019@gmail.com",
        to: req.body.email,
      ...
      transporter.sendMail(mailOptions, function (error, info) {
        if (error) {
          ...
          userFound.save();
          res.send(
            "<h1 style='color:rgb(23, 197, 23); position: absolute;left: 50%;transform: translateX(-50%); margin-top: 5vmax; text-align: center;'>Message Sent Successfully <br> <br> <a style='color: #000000;' href='/login'>Go Back</a></h1>"
          );
        }
      });
      // ***********************************
...
router.get("/change_password/:id", function (req, res, next) {
  ...
  .then((usr) =>{
    res.render("changPassword", { id: id , user : usr});
  })
  .catch(err => res.send(err));
  ...
  userDetail
    ...
    .then((usr) => {
      ...
      if (usr.resetToken === 1) {
        ...
        res.send(
          "<h1 style='color:red; position: absolute;left: 50%;transform: translateX(-50%); margin-top: 5vmax; text-align: center;'>Link Expired <br> <br> <a style='color: #000000;' href='/login'>Go to Login Page</a></h1>"
        );
      }
    })
    .catch((err) => res.send(err));
